...
def create_left_goods_log(doc, method):
    try:
        ...
        frappe.log_error(title="Error in create_left_goods_log", message=str(e))
        frappe.msgprint(f"Error: {str(e)}")


def update_left_goods_log(doc, method):
    if doc.reference_left_goods:
        left_goods_log = frappe.get_doc('Left Goods Log', doc.reference_left_goods)
...
def create_or_update_vehicle_log(doc, method):
    ...
    if vehicle_log:
        ...
        vehicle_log_doc.destination = doc.destination
